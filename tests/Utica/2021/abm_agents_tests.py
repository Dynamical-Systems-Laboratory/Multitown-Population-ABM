	...
	for agent in agents:
		...
		if workplaces[agent['workID']-1]['type'] == 'outside':
			...
			if not math.isclose(carpools[int(cID)-1].travel_time, agent['work travel time']):
				...
				print(carpools[int(cID)-1].travel_time, agent['work travel time'])
				return False
...
dpath = '../../../town_data/Utica/database/'
cpath = '../../../town_data/Utica/census_data/'
year = '2021'
...
pb_file_gis = dpath + year + '_core_poi_UticaIn_WorkTrimmed.csv'
# File with regular workplaces inside (with occupation)
pb_file_in = dpath + year + '_core_poi_UticaIn_OccupationTrimmed.csv'
# File with public places GIS data that are outside (with occupation)
pb_file_out = dpath + year + '_core_poi_UticaOut_WorkTrimmed.csv'
# File with building types
pb_type_file = dpath + 'public_types_mobility.txt'
# File with leisure inside of town
pb_leisure_file_in = dpath + year + '_core_poi_UticaIn_LeisureTrimmed.csv'
# File with leisure outside of town
pb_leisure_file_out = dpath + year + '_core_poi_UticaOut_LeisureTrimmed.csv'
...
# Zipcodes without public transit routes in the area
not_fpt_routes = dpath + 'not_public_transit.txt'
...
rh_out = 'Ut_retirement_homes.txt'
hs_out = 'Ut_households.txt' 
wk_out = 'Ut_workplaces.txt'
hsp_out = 'Ut_hospitals.txt'
sch_out = 'Ut_schools.txt'
ag_out = 'Ut_agents.txt'
cpool_out = 'Ut_carpool.txt'
public_out = 'Ut_public.txt'
leisure_out = 'Ut_leisure.txt'
...
# Transit
transit = travel.Transit(ftimes, fmodes, fcpools, fpt_routes, mode_speed, t_wfh, t_walk, not_fpt_routes) 
...
fout_NR = 'work_dist_Ut_count.txt'
